...
public class MyFactoryRepository <T,ID> implements ICrud<T,ID> {
     ...
     * AMA:: bize verilen bir nesne içindei dolu alanlara göre filtreleme yapmak.
     * Musteri nesnesi olduğunu düşünelim.
     * ad = "ahmet",adres = "Ankara" böyle bir nesnemiz olsun.
     * Burada içinde değer olan alanları seçip, içinde değer olmayan null olan alanları seçmiyoruz.
     * değer olan alanları kullanarak kriter oluşturacağız. Burada kriter için 2 değere ihtiyaç var.
    ...
    public List<T> findByEntity(T entity) { // {Musteri{id = null, ad="mur", adres="Ankara",telefon=null ...}}
        ...
        try {
            ...
            for (int i = 0; i < fl.length; i++) {
                ...
                /**
                 * Eğer okumakta olduğun aan null değilse ve aynı zamanda id kolonu değilse
                ...
                if(fl[i].get(entity)!=null && !fl[i].getName().equals("id")) {
                     ...
                     * String ad; -> DataType (String.class), DataName(ad), DataValue(muhammet)
                     * ad = "mu"
                     */
